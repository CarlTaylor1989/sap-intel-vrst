...
function getDb() {
	$db = new PDO('mysql:host=localhost;dbname=sapintel;charset=utf8', 'inteluser', 'X1mbFDEq');
...
function sendEmail($first_name, $last_name, $october6, $october7, $october8, $october9, $october10, $email)
{
	$mail = new PHPMailer(true);
		...
		try {
			$mail->SetFrom('noreply@virtualbigdatatour.com', 'SAP & Intel BIG DATA WEEK');

			$today = date("F j, Y, g:i a");	
			
			$mail->AddAddress($email, $first_name . ' ' . $last_name);
//			$mail->AddCC('izzy.p@dnxmarketing.com', 'Izzy Pink');

			
			$mail->Subject = 'Thanks for registering for SAP & Intel BIG DATA WEEK';
...
$mg->sendMessage($domain, array('from'    => 'noreply@virtualdatatour.com', 
                                'to'      => 'roberts_tom3@hotmail.com', 
                                'subject' => 'Thanks for registering for SAP & Intel BIG DATA WEEK - MAILGUN', 
                                'html'    => $emailBody));
